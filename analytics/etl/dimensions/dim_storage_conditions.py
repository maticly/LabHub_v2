...
def load_dim_conditions(duck_conn, dim_df: pd.DataFrame):
    ...
    logger.info("SCD1 for dw.Dim_Storage_Conditions...")
    duck_conn.register("tmp_dim_conditions", dim_df)

    # Update existing records
    duck_conn.execute(f"""
        UPDATE dw.Dim_Storage_Conditions
        SET
        ...
        WHERE dw.Dim_Storage_Conditions.StorageConditionID = tmp_dim_conditions.StorageConditionID
          AND (
              dw.Dim_Storage_Conditions.MaxTemp IS DISTINCT FROM tmp_dim_conditions.MaxTemp OR
              dw.Dim_Storage_Conditions.MinTemp IS DISTINCT FROM tmp_dim_conditions.MinTemp OR
              dw.Dim_Storage_Conditions.ConditionDescription IS DISTINCT FROM tmp_dim_conditions.ConditionDescription OR
              dw.Dim_Storage_Conditions.ConditionName IS DISTINCT FROM tmp_dim_conditions.ConditionName
          );
    """)

    # Insert new records
    duck_conn.execute(f"""
        INSERT INTO dw.Dim_Storage_Conditions (
        ...
        FROM tmp_dim_conditions
        WHERE NOT EXISTS (
            SELECT 1 FROM dw.Dim_Storage_Conditions
            WHERE dw.Dim_Storage_Conditions.StorageConditionID = tmp_dim_conditions.StorageConditionID
        );
    """)
...
def run_dim_conditions_etl(duck_conn):
    try:
        ...
        logger.info("✅ Dim_Storage_Conditions ETL completed successfully.")
    except Exception as e:
        logger.error(f"❌ Dim_Storage_Conditions ETL failed: {e}")
        raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        count = duck_conn.execute("SELECT COUNT(*) FROM dw.Dim_Storage_Conditions").fetchone()[0]
